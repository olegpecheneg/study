...
# Use shared dataprep package for config resolution
try:
    from dataprep.config import resolve_project_paths
except Exception:
    # fallback: local minimal resolver
    def resolve_project_paths():
        base = Path(__file__).resolve().parents[2]
        return {
            'PROJECT_ROOT': str(base),
            'CONSTRUCTS_DIR': str(base / 'MitoFragilityScore' / 'Constructs'),
            'ENERGIES_DIR': str(base / 'MitoFragilityScore' / 'Energies'),
            'OUTPUT_DIR': str(base / 'DataPreparing' / 'output'),
            'SEQUENCES_RELATIVE_DIR': str(base / 'MitoFragilityScore' / 'Sequences' / 'Relative'),
            'SNV_CSV': str(base / 'DataPreparing' / 'snv_csv' / 'snvs.csv'),
            'JOB_ID': ''
        }


_PROJECT_PATHS = resolve_project_paths()

# Import helper functions from dataprep package when available
try:
    from dataprep.snputils import load_snp_data, get_snps_in_construct, calculate_arm_ranges
    from dataprep.parsers import parse_construct_id
    from dataprep.stats import calculate_outlier_stats
    from dataprep.colors import generate_distinct_colors
    from dataprep.io import save_detailed_data, save_search_csv, save_master_json, write_outlier_stats
    from dataprep.plot_export import plot_energy_comparison as _plot_energy_comparison
    from dataprep.runner import (
        find_points_by_sequence as _find_points_by_sequence,
        find_individual_dirs as _find_individual_dirs,
        process_individual as _process_individual,
        search_sequences_wrapper as _search_sequences_wrapper,
        main as _runner_main
    )
except Exception:
    # If imports fail, keep local fallbacks (defined below in file)
    pass
...
try:
    # If dataprep.parsers provided parse_construct_id this will be used; otherwise keep local one defined below
    parse_construct_id  # type: ignore
except Exception:
    pass


try:
    calculate_arm_ranges  # provided by dataprep.snputils
    get_snps_in_construct
except Exception:
    pass
...
def plot_energy_comparison(ref_data: List[float], alt_data: List[float], 
    ...
    # JOB_ID prefix for outputs (optional)
    job_id = _PROJECT_PATHS.get('JOB_ID') if isinstance(_PROJECT_PATHS, dict) else ''
    ...
    detailed_data_path = Path(output_dir) / f"{job_prefix}{energy_type}_detailed_data.txt"
    try:
    ...
    search_data_path = Path(output_dir) / f"{job_prefix}{energy_type}_sequence_search.csv"
    search_data = []
    for i in range(min_len):
        details = construct_details[i]
    ...
    output_path = Path(output_dir) / f"{job_prefix}{energy_type}_comparison.png"
    ...
    try:
        ...
        html_path = Path(output_dir) / f"{job_prefix}{energy_type}_comparison.html"
        try:
    ...
    try:
        ...
        if len(ref_data) > 0:
            ...
            html_path_plotly = Path(output_dir) / f"{job_prefix}{energy_type}_comparison_plotly.html"
            try:
                config = {
                    'displayModeBar': True,
                    'scrollZoom': True,
                ...
                # Вставляем наш JSON-объект с детальными данными и JavaScript/CSS для модального окна перед закрывающим </body>
                try:
                ...
                combined_block = f"{json_script}{modal_js}{click_handler}"
                if '</body>' in html_content:
                    head, tail = html_content.rsplit('</body>', 1)
                    html_content = f"{head}{combined_block}</body>{tail}"
                else:
                    html_content = html_content + combined_block
...
def process_individual(ref_dir: str, alt_dir: str, snp_file_path: str, output_base_dir: str, individual_id: str) -> None:
    ...
    # Ищем CF файлы в директории конструктов для конкретного индивидуума
    constructs_base_dir = _PROJECT_PATHS.get('CONSTRUCTS_DIR')
    ...
    write_outlier_stats(outliers_stats, output_dir, individual_id, job_prefix=job_prefix)
...
def search_sequences_wrapper():
    try:
        return _search_sequences_wrapper()
    except Exception:
        import argparse
...
def main(base_dir: Optional[str] = None, output_base_dir: Optional[str] = None,
    ...
    try:
        return _runner_main(base_dir, output_base_dir, snp_base_dir, search_mode)
    except Exception:
        # Fallback: best-effort runner using available helpers
        if search_mode:
            search_sequences_wrapper()
            return
        if base_dir is None:
            base_dir = _PROJECT_PATHS.get('ENERGIES_DIR')
        if output_base_dir is None:
            output_base_dir = _PROJECT_PATHS.get('OUTPUT_DIR')
        if snp_base_dir is None:
            snp_base_dir = _PROJECT_PATHS.get('SEQUENCES_RELATIVE_DIR')

        ref_dir = os.path.join(base_dir, "SEQ-g38_Mt-Short_Test")
        os.makedirs(output_base_dir, exist_ok=True)
        individual_dirs = _find_individual_dirs(base_dir) if '_find_individual_dirs' in globals() else []
        ...
        for alt_dir, individual_id in individual_dirs:
            ...
            if not os.path.exists(snp_file_path):
                ...
                continue
            try:
                if '_process_individual' in globals():
                    _process_individual(ref_dir, alt_dir, snp_file_path, output_base_dir, individual_id)
                else:
                    logger.warning('Процесс обработки индивидуумов недоступен')
            except Exception as e:
